...
public class UserServiceImpl implements UserService{
    ...
    @Override
    public Mono<UserResponseDto> getByUserId(String userId) {
    ...
    public Mono<Void> saveUser(Mono<UserSaveDto> dtoMono) {
        return dtoMono.map(dto -> this.userRepository.saveUser(dto.toEntity()))
    ...
    public Mono<Void> updateUser(String userId, Mono<UserEditDto> dtoMono) {
        return dtoMono.map(dto-> this.userRepository.updateUser(dto.toEntity(userId)))
                        .and(ServerResponse.ok().build());
    }
